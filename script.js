...
function ensureChartJsIsReady(callback, attempt = 1) {
    ...
    // Annotation 플러그인은 Chart.js가 로드된 후, Chart.plugins.get('annotation') 등으로 확인하거나,
    // 또는 Chart.register(ChartAnnotation)을 시도해봐야 알 수 있습니다.
    // 여기서는 Chart와 Moment만 먼저 확인하고, Annotation은 차트 생성 직전에 등록 시도합니다.
    let annotationPotentiallyReady = typeof window.Chart !== 'undefined'; // Chart.js가 있어야 Annotation 등록 가능

    if (chartReady && momentReady) { // Annotation 플러그인 자체의 전역 객체 유무는 여기서 강제하지 않음
        console.log("Chart.js and Moment.js are ready. Annotation plugin will be registered before chart creation if available.");
        callback();
    } else {
        if (attempt <= MAX_ATTEMPTS) {
            console.warn(
                `Base libraries not ready yet (attempt ${attempt}/${MAX_ATTEMPTS}). Retrying in 200ms. Status:`,
                { chart: chartReady, moment: momentReady } // Annotation 제외하고 로깅
            );
            setTimeout(() => ensureChartJsIsReady(callback, attempt + 1), 200);
        } else {
            console.error("Failed to load Chart.js or Moment.js after multiple attempts.");
...
window.onload = () => {
    ...
    function createOrUpdatePriceChart(historicalData, fairValueResults) {
        ...
        // Annotation 플러그인 등록은 ensureChartJsIsReady에서 이미 처리했다고 가정
        // 또는 여기서 다시 한번 시도 (더 방어적으로)
        if (typeof window.Chart !== 'undefined' && typeof window.Chart.register === 'function') {
            if (typeof window.ChartAnnotation !== 'undefined') {
                try { Chart.register(window.ChartAnnotation); } catch(e) { console.warn("Annotation plugin (window.ChartAnnotation) registration failed in chart creation:", e); }
            } else if (typeof window.Chart.Annotation !== 'undefined') { // 대문자 A 확인
                try { Chart.register(window.Chart.Annotation); } catch(e) { console.warn("Annotation plugin (Chart.Annotation) registration failed in chart creation:", e); }
            } else {
                console.warn("ChartAnnotation plugin not found for registration in chart creation.");
            }
        } else {
             console.error('createOrUpdatePriceChart: Chart.js (Chart.register) is not available.');
             if(canvasEl.getContext('2d')) {
                const ctx = canvasEl.getContext('2d');
                ctx.clearRect(0, 0, canvasEl.width, canvasEl.height);
                ctx.font = "14px Arial"; ctx.textAlign = "center";
                ctx.fillText('차트 라이브러리 오류 (Chart.register)', canvasEl.width / 2, canvasEl.height / 2);
             }
             return;
        }
        ...
        if (containerWidth <= 0 || containerHeight <= 0) {
            console.warn("차트 컨테이너(#price-chart-wrapper)의 크기가 유효하지 않습니다.");
             if(canvasEl.getContext('2d')) { /* ... 이전과 동일 ... */ }
            return;
        }
